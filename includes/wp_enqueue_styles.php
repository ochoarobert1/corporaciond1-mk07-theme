<?php
function corporaciond1_load_css() {
    $version_remove = NULL;
    if (!is_admin()){
        if ($_SERVER['REMOTE_ADDR'] == '::1') {
            ...
            /*- FULLPAGE ON LOCAL -*/
            wp_register_style('fullpage-css', get_template_directory_uri() . '/css/fullpage.min.css', false, '3.0.4', 'all');
            ...
            /*- FULLPAGE -*/
            wp_register_style('fullpage-css', 'https://cdnjs.cloudflare.com/ajax/libs/fullPage.js/3.0.4/fullpage.min.css', false, '3.0.4', 'all');
            wp_enqueue_style('fullpage-css');
        }

        /*- GOOGLE FONTS -*/
        wp_register_style('google-fonts', 'https://fonts.googleapis.com/css?family=Titillium+Web:200,300,400,600,700,900', false, $version_remove, 'all');
        wp_enqueue_style('google-fonts');

        /*- MAIN STYLE -*/
        wp_register_style('main-style', get_template_directory_uri() . '/css/corporaciond1-style.css', false, $version_remove, 'all');
        wp_enqueue_style('main-style');

        /*- MAIN MEDIAQUERIES -*/
        wp_register_style('main-mediaqueries', get_template_directory_uri() . '/css/corporaciond1-mediaqueries.css', array('main-style'), $version_remove, 'all');
...
add_action('wp_enqueue_scripts', 'corporaciond1_load_css');
